...
def _make_ta(ema20=50000.0, ema50=48000.0, rsi14=60.0, atr14=1000.0):
    return TechnicalAnalysis(
        ema20=ema20, ema50=ema50, rsi14=rsi14, atr14=atr14, timestamp=datetime.now(UTC)
    )
...
def _make_futures(
    sentiment=SentimentLevel.NEUTRAL, funding_rate=0.0001, oi=1e10, ls=1.2, confidence=0.7
):
...
def _make_structure(
    structure=MarketStructure.BOS_BULLISH, direction=TrendDirection.BULLISH, sh=52000.0, sl=44000.0
):
...
class TestConfidenceEngine:
    ...
    def test_strong_bearish_aligned(self):
        ...
        result = engine.calculate(
            ...
            structure=_make_structure(
                structure=MarketStructure.BOS_BEARISH, direction=TrendDirection.BEARISH
            ),
            volume=_make_volume(),
            futures=_make_futures(),
            volatility=_make_volatility(),
            ...
            sentiment=_make_sentiment(
                overall=SentimentLevel.FEAR, fg_value=20, fg_label="Fear", news_score=-0.5
            ),
            price=50000.0,
        )
