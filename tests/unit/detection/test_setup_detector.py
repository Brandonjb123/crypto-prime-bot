...
class TestSetupDetector:
    ...
    def test_no_setup_when_none_triggered(self):
        ...
        snapshot = _make_snapshot(
            ...
            structure=MarketStructureResult(structure=MarketStructure.NONE, direction=TrendDirection.SIDEWAYS, swing_high=None, swing_low=None, timestamp=datetime.now(UTC)),
            volume=VolumeAnalysis(state=VolumeSignal.WEAK, spike_ratio=0.5, confidence_score=0.3, timestamp=datetime.now(UTC)),
            confidence=ConfidenceResult(score=0.40, level=ConfidenceLevel.LOW, positive_factors=[], negative_factors=["Test"], warnings=[ConfidenceWarning.LOW_VOLUME], blocked_reasons=[ConfidenceWarning.STRUCTURE_CONFLICT, ConfidenceWarning.LOW_VOLUME], timestamp=datetime.now(UTC)),
        )
        result = detector.detect(snapshot)
