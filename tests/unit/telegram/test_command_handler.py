...
from src.core.types.enums import (
    TelegramCommand, TelegramResponseType, PositionStatus,
    PositionCloseReason, Side, PortfolioStatus,
)
from src.telegram.command_handler import (
    status_handler, positions_handler, portfolio_handler, help_handler,
)
...
class TestCommandHandlers:
    def test_status_response(self):
        msg = TelegramMessage(chat_id="1", command=TelegramCommand.STATUS, timestamp=datetime.now(UTC))
    ...
    def test_positions_response(self):
        msg = TelegramMessage(chat_id="1", command=TelegramCommand.POSITIONS, timestamp=datetime.now(UTC))
        resp = positions_handler(msg, {"positions": [_make_position()]})
        assert "BTCUSDT" in resp.text

    def test_portfolio_response(self):
        msg = TelegramMessage(chat_id="1", command=TelegramCommand.PORTFOLIO, timestamp=datetime.now(UTC))
    ...
    def test_help_response(self):
        msg = TelegramMessage(chat_id="1", command=TelegramCommand.HELP, timestamp=datetime.now(UTC))
