...
from src.core.types.enums import (
    ConfidenceLevel, ExecutionAction, ExecutionStatus, ExecutionType,
    MarketStructure, OrderRejectReason, OrderStatus, PipelineStatus,
    PortfolioStatus, PositionCloseReason, PositionStatus,
    RecommendationAction, SentimentLevel, SetupType, Side,
    TrendDirection, ValidationCheck, VolumeSignal,
)
...
class TestApplicationPipeline:
    async def test_full_pipeline_integration(self):
        ...
        ta_engine.analyze.return_value = TechnicalAnalysis(
            ema20=50500.0, ema50=48000.0, rsi14=65.0, atr14=1000.0,
            timestamp=datetime.now(UTC),
        )
        ...
        vol_engine.analyze.return_value = VolumeAnalysis(
            state=VolumeSignal.SPIKE, spike_ratio=2.5,
            confidence_score=0.8, timestamp=datetime.now(UTC),
        )
        fut_engine = MagicMock()
        fut_engine.analyze.return_value = FuturesAnalysis(
            sentiment=SentimentLevel.GREED, funding_rate=0.0001,
            open_interest=1e10, long_short_ratio=1.2,
            confidence_score=0.7, timestamp=datetime.now(UTC),
        )
        vola_engine = MagicMock()
        vola_engine.analyze.return_value = VolatilityAnalysis(
            atr=1000.0, atr_normalized=2.0, risk_level="MEDIUM",
            confidence_score=1.0, timestamp=datetime.now(UTC),
        )
        sr_engine = MagicMock()
        sr_engine.analyze.return_value = SupportResistanceResult(
            nearest_support=45000.0, nearest_resistance=55000.0,
            price_position=0.3, confidence_score=0.8,
            timestamp=datetime.now(UTC),
        )
        sent_engine = MagicMock()
        sent_engine.analyze.return_value = SentimentAnalysis(
            overall=SentimentLevel.GREED, fear_greed_value=75,
            fear_greed_label="Greed", news_score=0.5,
            news_headline_count=1, confidence_score=0.8,
            timestamp=datetime.now(UTC),
        )
        conf_engine = MagicMock()
        conf_engine.calculate.return_value = ConfidenceResult(
            score=0.85, level=ConfidenceLevel.HIGH,
            positive_factors=["Test"], negative_factors=[],
        ...
        detector.detect.return_value = SetupResult(
            ...
            confidence_score=0.85, confidence_level=ConfidenceLevel.HIGH,
            blocked_reasons=[], is_valid_setup=True,
        ...
        risk_eng.calculate.return_value = RiskResult(
            ...
            direction=Side.LONG, risk_model="trend", warnings=[],
            timestamp=datetime.now(UTC),
        )
        rec_eng = MagicMock()
        rec_eng.recommend.return_value = RecommendationResult(
            action=RecommendationAction.BUY,
            summary="Test", reasons=[], warnings=[],
            confidence_score=0.85, confidence_level=ConfidenceLevel.HIGH,
            setup_type=SetupType.TREND_FOLLOWING, direction=Side.LONG,
            ...
            ready_for_execution=True, timestamp=datetime.now(UTC),
        )
        planner = MagicMock()
        planner.plan.return_value = ExecutionPlan(
            ...
            position_size=0.1, risk_reward_ratio=2.5, confidence_score=0.85,
            recommendation_action=RecommendationAction.BUY,
            summary="Test", blocked_reasons=[], validation_reasons=[],
            warnings=[], timestamp=datetime.now(UTC),
        )
        executor = MagicMock()
        executor.execute = AsyncMock(return_value=OrderResult(
